...
function TwitchActions(props) {
    const [liveChannelName, setLiveChannelName] = useState("")
    const [cmdChannelName, setCmdChannelName] = useState("")
    const [cmdCommandName, setCmdCommandName] = useState("")

    function createLive() {
        axios.post("https://karl-area-server.herokuapp.com/trigger/twitch/live", {
            'channel_name': liveChannelName,
        }).then((res) =>
    ...
    function createCMD() {
        axios.post("https://karl-area-server.herokuapp.com/trigger/twitch/command", {
            'channel_name': cmdChannelName,
            'commandname': cmdCommandName,
        }).then((res) =>
            props.setAction(res.data.user_trigger_id)
        ).catch((error) => {
            if (error.response) {
        ...
        <View style={styles.wrapperStyle}>
            <View style={styles.mainContainerStyle}>
                <ScrollView style={{marginLeft: 15, marginTop: 15}}>
                    ...
                    <Text style={styles.tableHeadTextStyle}>On Channel goes Live</Text>
                    <View style={styles.action}>
                        <TextInput
                            placeholder="Channel Name"
                            style={styles.textInput}
                            autoCapitalize="none"
                            onChangeText={(value) => setLiveChannelName(value)}
                        />
                    </View>
                    ...
                    <View style={styles.action}>
                        <TextInput
                            ...
                            style={styles.textInput}
                            autoCapitalize="none"
                            onChangeText={(value) => setCmdChannelName(value)}
                        />
                    </View>
                    <View style={styles.action}>
                        <TextInput
                            ...
                            style={styles.textInput}
                            autoCapitalize="none"
                            onChangeText={(value) => setCmdCommandName(value)}
                        />
                    </View>
                    <View style={styles.button}>
                        <TouchableOpacity
...
function TwitchReactions(props) {
    const [messageChannelName, setMessageChannelName] = useState("")
    const [messageString, setMessageString] = useState("")
    const [name, setName] = useState("")

    function createMessage() {
        axios.post("https://karl-area-server.herokuapp.com/reaction/twitch/message", {
                ...
                <ScrollView style={{marginLeft: 15, marginTop: 15}}>
                    ...
                    <View style={styles.action}>
                        <TextInput
                            ...
                            style={styles.textInput}
                            autoCapitalize="none"
                            onChangeText={(value) => setMessageChannelName(value)}
                        />
                    </View>
                    <View style={styles.action}>
                        <TextInput
...
export {TwitchActions, TwitchReactions}
